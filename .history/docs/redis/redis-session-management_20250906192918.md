...
This cache-backed authentication service acts as a **BFF for identity** — decoupling both the **frontend** and the **downstream APIs** from the complexity of Entra, tokens, and multi-tenant login.

- The **frontend** simply consumes a stable session, without needing to know about certificates, refresh tokens, or tenant switching.
- The **data APIs** no longer have to implement or understand MSAL or Entra authentication — they only receive validated, context-rich tokens or session claims from the BFF.

This separation allows APIs to stay **focused on business logic**, while the BFF centralizes identity, session management, and security enforcement.
...
- **Minh Phan** — Set up certificate service, and reccomended not to use innactivity logouts. And worked together to set up MSAL-Node.
  :::
