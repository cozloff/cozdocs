...
- **Tenant switching**: Different tenants require different authorities and certificates.
...
export const makeRedisCachePlugin = (tenantKey: string): ICachePlugin => ({
  beforeCacheAccess: async (ctx) => {
    const data = await redis.get(`msal-cache:${tenantKey}`);
    if (data) ctx.tokenCache.deserialize(data);
  },
  afterCacheAccess: async (ctx) => {
    if (ctx.cacheHasChanged) {
      await redis.set(`msal-cache:${tenantKey}`, ctx.tokenCache.serialize());
    }
  },
});
```
