...
Referencing across the infrastructure using data blocks:

```hcl
data "azurerm_container_app_environment" "aca_env" {
  name                = "app-env-global"      # must match the name from the first config
  resource_group_name = "rg-global-001"       # same RG where it was created
}
```
